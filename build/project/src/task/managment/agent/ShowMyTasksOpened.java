...
public class ShowMyTasksOpened extends Application {
	...
	public Map<Integer, String> getStatus() {
		...
		try {
			...
			statement = connection.prepareStatement("SELECT * FROM STATUS WHERE NAME = 'Opened'");
	...
	public List<Task> getTasks() {
		...
		try {
			...
			statement = connection.prepareStatement("SELECT * FROM Task WHERE STATUS_ID = ? AND USER_NAME = ?");
			statement.setInt(1, (int) status.keySet().iterator().next());
			statement.setString(2, LoginUserName.getLoginUserName());
			...
			while (result.next()) {
				Tasks.add(new Task(result.getString("NAME"), status.get(result.getInt("STATUS_ID")).toString()));
			}
